#!/usr/bin/env bash
# ari-bot log reader inner script. Runs as root via the pinned sudoers rule:
#   cmd_mcp ALL=(root) NOPASSWD: /opt/server-shell-mcp/scripts/read-ari-bot-log-inner.sh
#
# Usage: read-ari-bot-log-inner.sh <action> <file> <lines> [-e <keyword>]...
#   action:
...
LOG_ROOT="/var/www/html/ari-bot/logs"
MAX_LINES=2000
LIST_LIMIT=100
KEYWORD_MAX_LENGTH=128

fail() {
  echo "read-ari-bot-log: $1" >&2
  exit 2
}
...
LOG_ROOT_REAL="$(readlink -f -- "$LOG_ROOT")"

TARGET_REAL="$(readlink -f -- "$FILE_ARG")"
case "$TARGET_REAL" in
